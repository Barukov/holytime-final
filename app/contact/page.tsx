    ...
    <main className="min-h-screen bg-gradient-to-r from-[#eee8ff] via-white to-[#e8eeff] text-[#090522]">
      <header className="bg-[#13083d] px-8 py-8 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <Link href="/" className="rounded-full bg-[#6645e8] px-7 py-3 font-bold">
            Back home
          </Link>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-8 py-24 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <form onSubmit={handleSubmit} className="rounded-[30px] bg-white p-8 shadow-2xl">
          ...
          <div className="mt-8 space-y-5">
            <input name="name" placeholder="Your name" required className="w-full rounded-xl border border-black/15 px-5 py-4 outline-none focus:border-[#6541df]" />
            <input name="email" type="email" placeholder="Your email" required className="w-full rounded-xl border border-black/15 px-5 py-4 outline-none focus:border-[#6541df]" />
            <textarea name="message" placeholder="Your message" rows={7} required className="w-full rounded-xl border border-black/15 px-5 py-4 outline-none focus:border-[#6541df]" />

            <button type="submit" className="w-full rounded-2xl bg-[#6541df] px-8 py-4 font-black text-white transition hover:scale-[1.02]">
              Send message
            </button>
          </div>
        </form>
